...
在 Linux 下，要**查看当前目录下所有文件和文件夹占用的总磁盘空间**，这是一个典型的 `du` 使用场景。

查看当前目录下的各类文件或者文件夹的总的大小：
...
- `-s`：summary，只给出汇总
- `-h`：human-readable，人类可读（K/M/G）
- `.`：当前目录

查看当前目录下的各类文件或者文件夹的大小：
...
# 你在当前会话中，且能输入命令
screen -d
# 你在当前会话中，正在运行程序，无法输入命令
先按 Ctrl-A 然后松开，再按 D （Detach的缩写）
# 你在其他会话窗口，可指定分离该会话
screen -d 会话名或者ID
```
